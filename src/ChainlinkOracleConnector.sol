...
contract ChainlinkOracleConnector is Ownable {
    ...
    */
    ...
    /// Errors
    /// -----------------------------------------------------------------------
    error ChainlinkOracle_DATA_OUT_OF_DATE();
    error ChainlinkOracle_PRICE_ORACLE_UNAVAILABLE(address asset, address baseToken, address source);
    error ChainlinkOracle_INVALID_INPUT();
    error InvalidRound();
    ...
    */
    function updateDefaultChainlinkPriceAgeThreshold(uint256 _chainlinkPriceAgeThreshold) external onlyOwner {
        if (_chainlinkPriceAgeThreshold <= 1 minutes || _chainlinkPriceAgeThreshold >= 10 days) {
            revert ChainlinkOracle_INVALID_INPUT();
        }
    ...
    function updateChainlinkPriceAgeThreshold(address source, uint256 _chainlinkPriceAgeThreshold) external onlyOwner {
        if (_chainlinkPriceAgeThreshold <= 1 minutes || _chainlinkPriceAgeThreshold >= 10 days) {
            revert ChainlinkOracle_INVALID_INPUT();
        }
    ...
    */
    function getValue(address asset, address baseToken, uint256 amount) public view returns (uint256) {
        ...
        if (primarySource == address(0)) {
            revert ChainlinkOracle_PRICE_ORACLE_UNAVAILABLE(asset, baseToken, primarySource);
        }
    ...
    */
    ...
    function getPrice(address source) public view returns (uint256) {
        ...
        if (block.timestamp - updatedAt > ageThreshold) {
            revert ChainlinkOracle_DATA_OUT_OF_DATE();
        }
        if (price <= 0) {
            revert ChainlinkOracle_PRICE_ORACLE_UNAVAILABLE(address(source), address(0), address(0));
        }
        return uintprice;
    }
